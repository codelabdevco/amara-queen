  ...
  return (
    <AnimatePresence>
      ...
      >
        ...
        <motion.div
          className="relative bg-[#2a1215] border border-white/[0.02] rounded-2xl p-6 w-full max-w-[400px] max-h-[90vh] overflow-y-auto"
            ...
            {showSummary ? (
              ...
              <motion.div key="summary" className="space-y-4" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }}>
                <div className="text-center">
                  <div className="w-14 h-14 rounded-full bg-gold/10 border border-gold/[0.05] flex items-center justify-center mx-auto mb-3">
                ...
                {zodiac && (
                  <>
                    ...
                    {/* Personality */}
                    <div className="bg-[#1e0c0c] border border-gold/[0.02] rounded-xl p-3.5">
              ...
              <motion.div key="form" className="space-y-4" initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 30 }}>
                ...
                <div>
                  ...
                  <input type="text" placeholder="เช่น มายด์, เบส, แพร" value={form.nickname}
                    ...
                    className="w-full bg-[#1e0c0c] border border-white/[0.03] rounded-lg px-3 py-2.5 text-sm text-white placeholder:text-white/15 focus:border-gold/10 outline-none"
                    autoFocus
                  />
                </div>
                      ...
                      <button key={g.value} type="button"
                        ...
                        className={`py-2 rounded-lg border text-sm transition-all ${
                          form.gender === g.value
                            ...
                            : "bg-[#1e0c0c] border-gold/[0.02] text-white/40 hover:border-white/[0.03]"
                        }`}
                      >
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input type="tel" inputMode="numeric" placeholder="0812345678" value={form.phone}
                      ...
                      className="w-full bg-[#1e0c0c] border border-white/[0.03] rounded-lg px-3 py-2.5 text-sm text-white placeholder:text-white/15 focus:border-gold/10 outline-none font-mono tracking-wider"
                    />
                  </div>
                  <div>
                ...
                <div className="flex gap-3">
                  <button onClick={onClose} className="flex-1 py-2.5 rounded-lg border border-gold/[0.02] text-white/30 text-sm hover:text-white/50">
                    ข้าม
                  </button>
                  <button onClick={handleSave} disabled={saving}
                    className="flex-1 py-2.5 rounded-lg bg-gold/10 text-gold border border-gold/[0.05] text-sm font-medium hover:bg-gold/20 transition-colors disabled:opacity-30"
                  >
...
function InfoCard({ label, value, sub }: { label: string; value: string; sub?: string }) {
  return (
    <div className="bg-[#1e0c0c] border border-gold/[0.02] rounded-xl p-2.5 text-center">
