    ...
    >
      ...
      <div className="flex gap-1.5 flex-wrap justify-center mb-5 max-w-full">
        {pickedCards.map((card, i) => (
          <motion.button key={i}
            className={`w-[46px] h-[69px] rounded-md border border-gold/[0.05] overflow-hidden relative bg-[#1e0c0c] ${card.isReversed ? "rotate-180" : ""}`}
      ...
      <div className="w-full max-w-full space-y-4">
        ...
        {isLoadingAI && (
          <motion.div
            className="bg-gradient-to-br from-gold/[0.06] to-transparent border border-gold/[0.05] rounded-2xl p-5"
      ...
      {aiReading && (
        ...
        >
          ...
          <Button variant="outline" onClick={async () => {
            ...
            if (navigator.share) {
              try { await navigator.share({ title: "Amara Queen — ผลดูดวง", text }); } catch {}
            } else {
