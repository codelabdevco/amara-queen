...
export default function ShopScreen() {
  ...
  return (
    ...
    >
      {/* Cart button */}
      <motion.button
        className="fixed top-3 right-3 z-[110] w-8 h-8 rounded-full bg-gold/5 border border-white/[0.06] flex items-center justify-center text-white/40 active:bg-white/10 backdrop-blur-sm"
      ...
      <div className="grid grid-cols-2 gap-2.5 w-full max-w-md">
        {PRODUCTS.map((product, idx) => (
          <motion.div
            key={product.id}
            className="rounded-2xl border border-gold/[0.04] bg-[#2a1215]/90 overflow-hidden"
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: 0.1 + idx * 0.05, duration: 0.4, ease: EASE }}
          >
            {/* Image placeholder */}
            <div className="aspect-square bg-[#0a0b12] flex items-center justify-center border-b border-gold/[0.03]">
      ...
      <AnimatePresence>
        {showCart && (
          <>
            ...
            {/* Panel */}
            <motion.div
              className="fixed bottom-0 left-0 right-0 z-[130] bg-[#2a1215]/98 border-t border-white/[0.06] rounded-t-3xl max-h-[70vh] overflow-y-auto backdrop-blur-xl"
              initial={{ y: "100%" }}
              animate={{ y: 0 }}
              exit={{ y: "100%" }}
              ...
              <div className="p-5">
                ...
                {cart.length === 0 ? (
                  ...
                  <>
                    ...
                    {/* Total */}
                    <div className="flex justify-between items-center border-t border-gold/[0.04] pt-3 mb-5">
      ...
      <AnimatePresence>
        {showSuccess && (
          ...
          >
            <motion.div
              className="bg-[#2a1215] border border-white/[0.06] rounded-2xl p-8 flex flex-col items-center max-w-xs mx-4"
